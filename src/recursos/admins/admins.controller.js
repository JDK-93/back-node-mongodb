...
usuariosCtrl.createAdmins = async (req, res) => {
  ...
  dbConnection.run(response.sql, response.params, function (err) {
    if (err) {
      ...
      if (err.errno == 19) {
        logger.error(`Ya existe administrador registrado con el email [${req.body.email}]`)
        res.status(500).send(`Ya existe administrador registrado con el email [${req.body.email}]`)
      } else {
        logger.error(err)
      ...
      logger.info(`Administrador creado exitosamente para el email [${req.body.email}]`);
      res.status(200).json({
        admin: {
          name: req.body.name,
 ...
 // dbConnection.get(sql, [email], async (err, row) => {
    if (err) {
      ...
      if (row.rowCount > 0) {
        ...
        const validarPassword = await validatePassword(password, row.rows[0].password);
        if (!validarPassword) {
          logger.info(`Contraseña incorrecta para iniciar sesión con el email [${email}]`);
          return res.status(400).json(`Contraseña incorrecta para iniciar sesión con el email [${email}]`);
        } else {

...
// scp -r /home/jdk/git-jdk/app-fan-iot-postgres/servidor ubuntu@192.168.0.101:/home/ubuntu
const {
  newUsers,
  getUsers,
  deleteUsers
    ...
    if (err) {
      ...
      if (resql.rowCount > 0) {
        if(sql.rfid){
          ...
          resql.rows[0].edad = Math.floor(moment(new Date()).diff(moment(resql.rows[0].date,"YYYY-MM-DD"),'years',true))//2020-10-14
...
usuariosCtrl.createUsuarios = async (req, res) => {
  ...
  dbConnection.query(sql.text, sql.values, (err, resql) => {
    if (err) {
      ...
      res.status(200).json({ "message": `Usuario ${req.body.name} añadido correctamente.` })
    }
  })
};
...
// UPDATE usuario 
// Para editar, tengo que enviar todos los parámetros es que tengo que manda todos los parámetros
usuariosCtrl.updateUsuario = async (req, res) => {
  const {userid, rfid, dni, name, telefono, email, sexo, date,grupo, admin } = req.body;
  console.log("Editando: ___________________________________--")
  console.log(req.body)
  dbConnection.query(`UPDATE users SET rfid = $1, dni =$2, name=$3, telefono=$4, email=$5, sexo=$6, date=$7, grupo=$8, adminId=$9 WHERE userid =$10`, [rfid, dni, name, telefono, email, sexo, date,grupo, admin, userid], function (err, rows) {
    if (err) {
      logger.info(err);
      logger.info(`Usuario inexistente, Se paso un token con formato invalido, o no esta asignado a un usuario`);
      return res.status(404).send("El Usuario no fue encontrado");
    } else {
      ...
      if (rows.rowCount > 0) {
        ...
        res.status(200).send(`Usuario con dni ${dni} actualizado correctamente.`)
      }
      else {

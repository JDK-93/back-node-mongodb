...
let validarUsuarios = (req, res, next) => {
    ...
    if (resultado.error === null) {
        ...
        logger.info("Usuario no puedo ser validado", resultado.error.details.map(error => error.message))
        res.status(400).send(`Tu usuario no cumple requisitos: [${erroresDeValidacion}]`)
...
let validarPedidoDeLogin = (req, res, next) => {
    ...
    if (resultado.error === null) {
        ...
        res.status(400).send("Login fallo. Debes especificar email y contraseña de usuario.")
    }
}
